...
const STORAGE_KEY_3C = 'sakip_kriteria_3c_v2';

export const Pelaporan3cView: React.FC<Pelaporan3cViewProps> = ({
  opdList = [],
  selectedOpdId,
  selectedYear,
  currentUser,
  ...
  const nilai3c = Math.round(((persenPemenuhan / 100) * bobot3c) * 100) / 100;
  ...
  const handleToggleSkor = (id: string) => {
    ...
    setSaveToast('Skor kriteria 3.c berhasil diperbarui');
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    setEditingItem(null);
    setSaveToast('Bukti dukung kriteria 3.c berhasil disimpan!');
    setTimeout(() => setSaveToast(null), 3000);
  };

  const handleResetDefault = () => {
    if (confirm('Kembalikan data Kriteria 3.c ke default sistem?')) {
      setKriteriaList(INITIAL_KRITERIA_3C);
      localStorage.removeItem(STORAGE_KEY_3C);
      setSaveToast('Data kriteria 3.c direset ke default');
      setTimeout(() => setSaveToast(null), 2500);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 md:p-6">
          ...
          <div className="space-y-1.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-md bg-amber-100 text-amber-900 font-bold text-xs">
                Sub-Komponen 3.c (Bobot Terbesar)
              </span>
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              3.c Pemanfaatan Laporan Kinerja & Pengaruh Budaya Akuntabilitas
            </h1>
          ...
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl shrink-0">
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">
                Capaian Sub-Komponen 3.c
              </span>
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-2xl font-black text-slate-900 font-mono">
                  {nilai3c.toFixed(2)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">/ {bobot3c.toFixed(2)}</span>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setActiveTabSub('dampak')}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${
                activeTabSub === 'dampak'
                  ? 'bg-amber-600 text-white shadow-xs'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
            ...
            >
              ...
              <span>Dampak Manajerial & Disposisi Pimpinan</span>
            </button>
          </div>
      ...
      {/* TAB 2: DAMPAK MANAJERIAL & DISPOSISI */}
      {activeTabSub === 'dampak' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              Pemanfaatan Nyata Rekomendasi LKjIP bagi Keputusan Manajemen
            </h2>
            <p className="text-xs text-slate-500">
              Tindak lanjut atas capaian tahunan yang mengubah strategi operasional dan alokasi anggaran rumah sakit.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Arahan Kebijakan Perbaikan Berkelanjutan</h3>
              <p className="text-xs text-slate-600">
                Direktur Utama menerbitkan 8 poin instruksi tindak lanjut perbaikan atas indikator response time IGD dan kepuasan pasien rawat jalan.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                ND.01.02/DIRUT/TINDAK-LANJUT/2025/08
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Pengalihan Alokasi ke Program Prioritas</h3>
              <p className="text-xs text-slate-600">
                Hasil laporan kinerja mendasari pergeseran anggaran RBA BLU sebesar Rp 1.5 Miliar untuk penambahan mesin antrean digital dan penguatan SDM IT.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                SK-REVISI-RBA-03/2025
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-300">Sub-Komponen 3.c: Pemanfaatan Laporan Kinerja</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Lembar Disposisi Direktur Utama atas Evaluasi LKjIP..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Direktur Utama & Tim Terkait"
                    className="w-full text-xs rounded-lg border border-slate-200 p-2 focus:ring-2 focus:ring-amber-500"
                  />
                </div>
