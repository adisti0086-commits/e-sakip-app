...
const STORAGE_KEY_4B = 'sakip_kriteria_4b_v2';

export const Evaluasi4bView: React.FC<Evaluasi4bViewProps> = ({
  opdList = [],
  selectedOpdId,
  selectedYear,
  currentUser,
  ...
  const nilai4b = Math.round(((persenPemenuhan / 100) * bobot4b) * 100) / 100;
  ...
  const handleToggleSkor = (id: string) => {
    ...
    setSaveToast('Skor kriteria 4.b berhasil diperbarui');
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    setEditingItem(null);
    setSaveToast('Bukti dukung kriteria 4.b berhasil disimpan!');
    setTimeout(() => setSaveToast(null), 3000);
  };

  const handleResetDefault = () => {
    if (confirm('Kembalikan data Kriteria 4.b ke default sistem?')) {
      setKriteriaList(INITIAL_KRITERIA_4B);
      localStorage.removeItem(STORAGE_KEY_4B);
      setSaveToast('Data kriteria 4.b direset ke default');
      setTimeout(() => setSaveToast(null), 2500);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 md:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1.5">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              4.b Evaluasi Akuntabilitas Kinerja Memberikan Nilai Tambah & Berkualitas
            </h1>
          ...
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl shrink-0">
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">
                Capaian Sub-Komponen 4.b
              </span>
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-2xl font-black text-slate-900 font-mono">
                  {nilai4b.toFixed(2)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">/ {bobot4b.toFixed(2)}</span>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setActiveTabSub('kompetensi')}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${
                activeTabSub === 'kompetensi'
                  ? 'bg-indigo-600 text-white shadow-xs'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
              }`}
            >
              <CheckCheck className="w-4 h-4" />
              <span>Sertifikasi Auditor SPI & Kertas Kerja</span>
            </button>
          </div>
      ...
      {/* TAB 2: SERTIFIKASI AUDITOR & KERTAS KERJA */}
      {activeTabSub === 'kompetensi' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              Standar Kompetensi Auditor SPI & Instrumen Kertas Kerja Evaluasi (KKE)
            </h2>
            <p className="text-xs text-slate-500">
              Validasi profesionalisme evaluator independen internal rumah sakit.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Sertifikat Diklat Evaluasi AKIP Pusdiklatwas</h3>
              <p className="text-xs text-slate-600">
                4 orang Auditor SPI telah lulus sertifikasi evaluator AKIP Pusdiklatwas BPKP Ciawi dan terdaftar aktif dalam asosiasi auditor internal pemerintah.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                No Sertifikat: BPKP/PUSDIK/AKIP/2023/114
              </div>
            </div>

            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              <span className="px-2 py-0.5 rounded bg-indigo-100 text-indigo-900 font-bold text-[10px]">
                KERTAS KERJA STANDAR PERMENPAN
              </span>
              <h3 className="text-sm font-bold text-slate-900">Aplikasi KKE Elektronik SPI</h3>
              <p className="text-xs text-slate-600">
                Lembar Kerja Evaluasi menggunakan format baku PermenPAN-RB No. 88/2021 dengan bobot komponen terverifikasi dan matriks uji petik dokumen.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                KKE-SPI-DJAMIL-V2/2024
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-300">Sub-Komponen 4.b: Kualitas Evaluasi Internal</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Sertifikat Diklat Evaluasi AKIP Pusdiklatwas BPKP..."
