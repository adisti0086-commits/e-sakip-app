...
import { INITIAL_KRITERIA_4C, SakipKriteriaItem } from '../../data/sakipKompData';
import { OPD, User } from '../types';
import { notifyLKESync, LKE_SYNC_EVENT } from '../utils/lkeSync';

interface Evaluasi4cViewProps {
  opdList: OPD[];
...
const STORAGE_KEY_4C = 'sakip_kriteria_4c_v2';

export const Evaluasi4cView: React.FC<Evaluasi4cViewProps> = ({
  opdList = [],
  selectedOpdId,
  selectedYear,
  currentUser,
  ...
  const [activeTabSub, setActiveTabSub] = useState<'kriteria' | 'tlr'>('kriteria');

  const [kriteriaList, setKriteriaList] = useState<SakipKriteriaItem[]>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY_4C);
      if (saved) return JSON.parse(saved);
    } catch {
      // fallback
    }
    return INITIAL_KRITERIA_4C;
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_4C, JSON.stringify(kriteriaList));
  ...
  useEffect(() => {
    const handleSync = (e: Event) => {
      ...
      if (ce.detail?.source === '4.c') return;
      try {
        const saved = localStorage.getItem(STORAGE_KEY_4C);
        if (saved) {
  ...
  const nilai4c = Math.round(((persenPemenuhan / 100) * bobot4c) * 100) / 100;
  ...
  const handleToggleSkor = (id: string) => {
    ...
    setSaveToast('Skor kriteria 4.c berhasil diperbarui');
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    setEditingItem(null);
    setSaveToast('Bukti dukung kriteria 4.c berhasil disimpan!');
    setTimeout(() => setSaveToast(null), 3000);
  };

  const handleResetDefault = () => {
    if (confirm('Kembalikan data Kriteria 4.c ke default sistem?')) {
      setKriteriaList(INITIAL_KRITERIA_4C);
      localStorage.removeItem(STORAGE_KEY_4C);
      setSaveToast('Data kriteria 4.c direset ke default');
      setTimeout(() => setSaveToast(null), 2500);
    }
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-1.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-md bg-indigo-100 text-indigo-900 font-bold text-xs">
                Sub-Komponen 4.c (Bobot Terbesar)
              </span>
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              4.c Pemanfaatan Hasil Evaluasi Internal (Tindak Lanjut Rekomendasi)
            </h1>
            <p className="text-xs text-slate-500 max-w-3xl leading-relaxed">
              Memastikan 100% rekomendasi SPI ditindaklanjuti secara nyata oleh unit kerja, kenaikan tren skor SAKIP, perbaikan efisiensi operasional, dan kepatuhan unit kerja dalam akuntabilitas kinerja.
            </p>
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-900 border border-indigo-300 text-xs font-semibold">
                ...
                Otomatis Terhubung ke LKE • Nilai: {nilai4c.toFixed(2)} / {bobot4c.toFixed(2)}
              </span>
              {onNavigateTab && (
                <button
                  type="button"
          ...
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl shrink-0">
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">
                Capaian Sub-Komponen 4.c
              </span>
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-2xl font-black text-slate-900 font-mono">
                  {nilai4c.toFixed(2)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">/ {bobot4c.toFixed(2)}</span>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Matriks Tindak Lanjut Rekomendasi (TLR)</span>
            </button>
          </div>
      ...
      {/* TAB 2: MATRIKS TINDAK LANJUT REKOMENDASI (TLR) */}
      {activeTabSub === 'tlr' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              Matriks Pemantauan Tindak Lanjut Rekomendasi (TLR) Hasil Evaluasi SPI & Kemenkes
            </h2>
            <p className="text-xs text-slate-500">
              Persentase penyelesaian rekomendasi perbaikan akuntabilitas kinerja rumah sakit mencapai 100%.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              <span className="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 font-bold text-[10px]">
                STATUS TL REKOMENDASI
              </span>
              <h3 className="text-sm font-bold text-slate-900">100% Rekomendasi Selesai</h3>
              <p className="text-xs text-slate-600">
                Seluruh 12 poin rekomendasi LHE SAKIP tahun sebelumnya telah tuntas ditindaklanjuti dan diverifikasi tuntas oleh Tim Verifikator SPI.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                BA-VERIFIKASI-TLR/2024/09
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Kenaikan Nilai SAKIP Tahunan</h3>
              <p className="text-xs text-slate-600">
                Nilai SAKIP RSUP Dr. M. Djamil konsisten meningkat dari 82.15 (Tahun 2022) menjadi 84.80 (Tahun 2023) dan ditargetkan &gt; 86.50 (Tahun 2024).
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                LHE Itjen Kemenkes RI
              </div>
            </div>

            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              <span className="px-2 py-0.5 rounded bg-purple-100 text-purple-800 font-bold text-[10px]">
                KEPATUHAN UNIT KERJA
              </span>
              <h3 className="text-sm font-bold text-slate-900">100% Kepatuhan Pengukuran</h3>
              <p className="text-xs text-slate-600">
                Seluruh 26 instalasi dan bagian tepat waktu menyampaikan laporan capaian bulanan dan triwulanan tanpa ada teguran administratif.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                RAPOR-KEPATUHAN-UNIT/2024
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-300">Sub-Komponen 4.c: Pemanfaatan Hasil Evaluasi</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Matriks Tindak Lanjut Rekomendasi (TLR) SPI..."
