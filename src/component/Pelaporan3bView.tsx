...
const STORAGE_KEY_3B = 'sakip_kriteria_3b_v2';

export const Pelaporan3bView: React.FC<Pelaporan3bViewProps> = ({
  opdList = [],
  selectedOpdId,
  selectedYear,
  currentUser,
  ...
  const nilai3b = Math.round(((persenPemenuhan / 100) * bobot3b) * 100) / 100;
  ...
  const handleToggleSkor = (id: string) => {
    ...
    setSaveToast('Skor kriteria 3.b berhasil diperbarui');
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    setEditingItem(null);
    setSaveToast('Bukti dukung kriteria 3.b berhasil disimpan!');
    setTimeout(() => setSaveToast(null), 3000);
  };

  const handleResetDefault = () => {
    if (confirm('Kembalikan data Kriteria 3.b ke default sistem?')) {
      setKriteriaList(INITIAL_KRITERIA_3B);
      localStorage.removeItem(STORAGE_KEY_3B);
      setSaveToast('Data kriteria 3.b direset ke default');
      setTimeout(() => setSaveToast(null), 2500);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 md:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1.5">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              3.b Laporan Kinerja Memberikan Informasi Terukur & Analisis Mendalam
            </h1>
          ...
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl shrink-0">
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">
                Capaian Sub-Komponen 3.b
              </span>
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-2xl font-black text-slate-900 font-mono">
                  {nilai3b.toFixed(2)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">/ {bobot3b.toFixed(2)}</span>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setActiveTabSub('kualitas')}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${
                activeTabSub === 'kualitas'
                  ? 'bg-amber-600 text-white shadow-xs'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
              }`}
            >
              <GitBranch className="w-4 h-4" />
              <span>Standar Kualitas & Benchmark Nasional</span>
            </button>
          </div>
      ...
      {/* TAB 2: STANDAR KUALITAS & BENCHMARK */}
      {activeTabSub === 'kualitas' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              Standar Kedalaman Informasi & Analisis Performa LKjIP
            </h2>
            <p className="text-xs text-slate-500">
              Indikator kualitas pelaporan kinerja sesuai standar PermenPAN-RB No. 88/2021.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Analisis 3 Tahun Terakhir</h3>
              <p className="text-xs text-slate-600">
                Membandingkan capaian tahun 2022, 2023, dan 2024 untuk melihat laju pertumbuhan kepuasan pasien dan stabilitas rasio keuangan BLU.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                Bab III LKjIP Hal 45-52
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Komparasi Rumah Sakit Vertikal</h3>
              <p className="text-xs text-slate-600">
                Membandingkan capaian mutu klinis dan waktu tunggu rawat jalan dengan rata-rata RSUP rujukan nasional lainnya di bawah Ditjen Pelayanan Kesehatan.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                Bab III LKjIP Hal 68-75
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Diagram Fishbone & Akar Masalah</h3>
              <p className="text-xs text-slate-600">
                Uraian mendalam atas indikator yang belum mencapai 100% target, membedah kendala dari aspek Man, Machine, Method, Material, dan Money.
              </p>
              <div className="text-[10px] font-mono text-slate-500 bg-white p-2 rounded border border-slate-200">
                Bab III LKjIP Hal 80-92
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-300">Sub-Komponen 3.b: Kualitas Informasi LKjIP</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Bab III LKjIP Memuat Komparasi Tren Multi-Tahun..."
