...
const STORAGE_KEY_2B = 'sakip_kriteria_2b_v2';

export const Pengukuran2bView: React.FC<Pengukuran2bViewProps> = ({
  opdList = [],
  selectedOpdId,
  selectedYear,
  currentUser,
  ...
  const [upayaInovatif2b, setUpayaInovatif2b] = useState<'Dapat' | 'Tidak Dapat'>(() => {
    try {
      const saved = localStorage.getItem('sakip_2b_upaya_inovatif');
  ...
  const nilai2b = Math.round(((persenPemenuhan / 100) * bobot2b) * 100) / 100;
  ...
  const handleToggleSkor = (id: string) => {
    ...
    setSaveToast('Skor kriteria 2.b berhasil diperbarui');
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    setEditingItem(null);
    setSaveToast('Bukti dukung kriteria 2.b berhasil disimpan!');
    setTimeout(() => setSaveToast(null), 3000);
  };

  const handleResetDefault = () => {
    if (confirm('Kembalikan data Kriteria 2.b ke default sistem?')) {
      setKriteriaList(INITIAL_KRITERIA_2B);
      localStorage.removeItem(STORAGE_KEY_2B);
      setSaveToast('Data kriteria 2.b direset ke default');
      setTimeout(() => setSaveToast(null), 2500);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1.5">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              2.b Pengukuran Kinerja Telah Menjadi Kebutuhan dalam Pengelolaan Kinerja
            </h1>
          ...
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-4 rounded-xl shrink-0">
            <div className="text-right">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">
                Capaian Sub-Komponen 2.b
              </span>
              <div className="flex items-baseline gap-1 justify-end">
                <span className="text-2xl font-black text-slate-900 font-mono">
                  {nilai2b.toFixed(2)}
                </span>
                <span className="text-xs text-slate-400 font-semibold">/ {bobot2b.toFixed(2)}</span>
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Siklus Pemantauan & Pemanfaatan Aplikasi IT</span>
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {filteredKriteria.map((item) => {
              const isSubstantiveGreen = item.no === 2 || item.no === 3;

              return (
                <div
                  key={item.id}
                  className={`rounded-xl border shadow-xs transition-colors p-4 md:p-5 space-y-3 ${
                    isSubstantiveGreen
                      ? 'bg-[#dcedc8]/40 border-emerald-400 ring-1 ring-emerald-300/60'
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <span
                        className={`px-2 py-1 rounded-md font-mono font-black text-xs shrink-0 ${
                          isSubstantiveGreen
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {isSubstantiveGreen && (
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-900 border border-emerald-300">
                              Kriteria Kunci Relevansi & Dukungan (Baris Hijau LKE)
                            </span>
                          )}
                        </div>
                  ...
                  <div className="p-3 bg-slate-50/90 rounded-lg border border-slate-200 text-xs space-y-2">
          ...
          <div className="bg-[#b3d4fc]/30 border-2 border-sky-300 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="text-sm font-black text-slate-900">
                    Terdapat upaya inovatif terkait kualitas pengukuran kinerja
                  </h4>
                ...
                <p className="text-xs text-slate-600 mt-0.5">
                  Pengembangan modul otomasi perhitungan capaian SIMRS dan portal SAKElek real-time.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 self-end sm:self-center bg-white px-3.5 py-2 rounded-xl border border-sky-300 shadow-2xs">
              ...
              <select
                value={upayaInovatif2b}
                disabled={!canEdit}
                onChange={(e) => {
                  const val = e.target.value as any;
                  setUpayaInovatif2b(val);
                  localStorage.setItem('sakip_2b_upaya_inovatif', val);
                  setSaveToast(`Status Inovasi 2.b disimpan: ${val}`);
                  setTimeout(() => setSaveToast(null), 2500);
                }}
      ...
      {/* TAB 2: SIKLUS PEMANTAUAN & PEMANFAATAN IT */}
      {activeTabSub === 'pemantauan' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-5">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              Siklus Pemantauan Kinerja & Pemanfaatan Teknologi Informasi
            </h2>
            <p className="text-xs text-slate-500">
              Dokumentasi integrasi aplikasi SIMRS, modul input SAKElek, dan alur pelaporan berjenjang pimpinan.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Keterlibatan Pimpinan (RTM)</h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                Direksi memimpin Rapat Tinjauan Manajemen (RTM) setiap bulan untuk mengevaluasi gap capaian IKU dan menandatangani laporan triwulanan.
              </p>
              <span className="inline-block text-[10px] font-mono text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">
                ND.01.02/D.XVI/RTM/024/2024
              </span>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Monitoring Berjenjang Unit</h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                Pemantauan capaian dilakukan berjenjang: Kepala Ruangan &gt; Sub-Koordinator &gt; Koordinator Bagian &gt; Direktur Bidang terkait.
              </p>
              <span className="inline-block text-[10px] font-mono text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">
                FORM-MON-ORG/2024/09
              </span>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Pemanfaatan IT & SIMRS</h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                Pengumpulan dan pengukuran data dilakukan otomatis melalui modul digital SAKElek Kemenkes yang terintegrasi dengan database SIMRS.
              </p>
              <span className="inline-block text-[10px] font-mono text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200">
                SK-SIMAKIP-09/2024
              </span>
            ...
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-300">Sub-Komponen 2.b: Pengukuran Kinerja</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Notula RTM Pimpinan Direksi..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Sekretariat Pimpinan / Direksi"
                    className="w-full text-xs rounded-lg border border-slate-200 p-2 focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
